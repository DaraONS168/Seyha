...
export default function FuelExpensesPage() {
  ...
  const save = async event => {
    ...
    setSaving(true)
    const result = await fuelExpenseService.create(form, files).catch(error => ({ error }))
    setSaving(false)
  ...
  return <div className="space-y-5">
    ...
    <Modal open={Boolean(form)} onClose={() => setForm(null)} title="កត់ត្រាចំណាយសាំង" size="max-w-4xl"><form className="space-y-5" onSubmit={save}>
      <FormSection number="1" title="បុគ្គលិក និងផែនការចុះទីតាំង" description="ជ្រើស Sales ជាមុន ដើម្បីបង្ហាញតែផែនការរបស់បុគ្គលិកនោះ។"><div className="grid gap-4 md:grid-cols-2">
      ...
      <FormSection number="2" title="ទីតាំង និងកាលបរិច្ឆេទ" description="កំណត់តំបន់ដែលបានចុះបំពេញការងារ។"><div className="grid gap-4 md:grid-cols-2">
        ...
        <Field label="កាលបរិច្ឆេទ *"><input required type="date" className="field" value={form?.expense_date || ''} onChange={event => setForm({ ...form, expense_date: event.target.value })}/></Field>
