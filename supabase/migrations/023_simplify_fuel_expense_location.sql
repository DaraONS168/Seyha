-- Keep the expense request link internal and use province/district as the visible location.
...
begin
  ...
  else new.expense_code:=old.expense_code; if old.status not in ('draft','rejected') and row(new.start_odometer,new.end_odometer,new.fuel_liters,new.price_per_liter,new.invoice_number,new.province_id,new.district_id) is distinct from row(old.start_odometer,old.end_odometer,old.fuel_liters,old.price_per_liter,old.invoice_number,old.province_id,old.district_id) then raise exception 'Only draft or rejected fuel expense can be edited'; end if; end if;
  return new;
end $$;
