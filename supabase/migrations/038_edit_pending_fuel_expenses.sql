-- Let fuel managers correct pending fuel expenses before approval.

drop policy if exists fuel_expenses_permission_update on public.fuel_expenses;
create policy fuel_expenses_permission_update on public.fuel_expenses for update to authenticated
using(
  public.has_permission('fuel.update')
  and status in ('draft','rejected','submitted')
  and (created_by = auth.uid() or public.has_permission('fuel.approve'))
)
with check(
  public.has_permission('fuel.update')
  and (created_by = auth.uid() or public.has_permission('fuel.approve'))
...
begin
  ...
  if tg_op='INSERT' then
    ...
    if old.status not in ('draft','rejected','submitted') and row(new.start_odometer,new.end_odometer,new.fuel_liters,new.price_per_liter,new.invoice_number,new.province_id,new.district_id,new.currency) is distinct from row(old.start_odometer,old.end_odometer,old.fuel_liters,old.price_per_liter,old.invoice_number,old.province_id,old.district_id,old.currency) then
      raise exception 'Only draft, rejected or submitted fuel expense can be edited';
    end if;
  end if;
  return new;
end $$;
